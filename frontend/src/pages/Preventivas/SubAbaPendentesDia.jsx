...
const SubAbaPendentesDia = ({ setSidebarContent, globalFilters, currentUser, clearFiltersTrigger, isActive }) => {
    ...
    useEffect(() => {
        const fetchFilteredData = async () => {
            ...
            try {
                ...
                const [detalhesEmDiaRes, kpisRes] = await Promise.all([
                ...
                if (!detalhesEmDiaRes.ok || !kpisRes.ok) throw new Error(`Falha ao buscar dados detalhados.`);
    ...
    return (
        <>
            <div className="analysis-block">
                ...
                <div className={`analysis-block-content ${isAccordionOpen ? '' : 'collapsed'}`}>
                    <div className="dashboard-grid full-width-grid">
                         <ChartWrapper title={`Quantitativo por ${visao}`} onPhotoModeClick={() => setChartInModal({ title: `Pendentes em Dia por ${visao}`, chart: <GenericBarChart data={pendentesEmDiaData} dataKey="count" xAxisKey="nome_grupo" chartColor="#e87722" /> })} isFilterSource={filterSourceChart === 'emDiaChart'} kpiCards={[{ title: 'Em Dia', value: kpis.total_em_dia || 0 }]}>
                            {isLoading ? <p>Carregando...</p> : error ? <p style={{ color: 'red' }}>{error}</p> : <GenericBarChart data={pendentesEmDiaData} dataKey="count" xAxisKey="nome_grupo" chartId="emDiaChart" onClickHandler={handleChartClick} chartColor="#e87722" />}
                        </ChartWrapper>
                    </div>
                    <TableWrapper title="Detalhes das Preventivas em Dia" onExpandClick={() => setIsTableModalOpen(true)}>
            ...
            <TableModal isOpen={isTableModalOpen} onClose={() => setIsTableModalOpen(false)} title="Detalhamento Avançado - Preventivas em Dia" columns={tableColumns} rows={tabelaEmDiaData}/>
        </>
    );
};

export default SubAbaPendentesDia;
